...
adition_point_std_values = [0.1, 0.3464, 0.6, 1, 1.5, 2]

ranges = [x1_range, x2_range]
initial_point = llm_generate_initial_points(ranges, num_points=20)
results = {}

for std_val in adition_point_std_values:
    print(f"\n--- 正在测试 adition_point_std: {std_val} ---")
    best_points = []
    best_costs = []
    best_rels_list = []
    iterations_list = []
    ...
    for i in range(num_runs_per_setting):
        print(f"\n--- 正在运行第 {i+1} 次 --- (adition_point_std: {std_val})")
        best_point, best_cost, best_reliabilities, best_penalty, actual_iterations = llm_optimize_with_llm(
            ...
            std=std_dev,
            adition_point_std=np.array([std_val, std_val]),
            model=model,
    ...
    print(f"\n--- adition_point_std {std_val} 的统计结果 ---")
...
print("\n所有 adition_point_std 对比实验完成！")
print("最终统计结果:")
print("=" * 130)
print(f"{'扰动标准差':<10} {'平均最优点':<10} {'最优点标准差':<10} {'平均成本':<10} {'成本标准差':<10} {'平均迭代':<10} {'迭代标准差':<10} {'AvgRel G1':<10} {'G2':<10} {'G3':<10}")
print("-" * 130)

for std_val in adition_point_std_values:
    res = results[str(std_val)]
    ...
    print(f"{std_val:<12} {avg_point_str:<10} {std_point_str:<10} {res['avg_best_cost']:<10.4f} {res['std_best_cost']:<10.4f} {res['avg_iterations']:<10.2f} {res['std_iterations']:<10.2f} {res['avg_reliability_G1']:<10.4f} {res['avg_reliability_G2']:<10.4f} {res['avg_reliability_G3']:<10.4f}")

perturbation_stds = adition_point_std_values
optimal_costs = [results[str(n)]["avg_best_cost"] for n in perturbation_stds]
optimal_costs_std = [results[str(n)]["std_best_cost"] for n in perturbation_stds]
iteration_counts = [results[str(n)]["avg_iterations"] for n in perturbation_stds]
iteration_counts_std = [results[str(n)]["std_iterations"] for n in perturbation_stds]
...
ax1.set_xticks(perturbation_stds)
ax1.set_xticklabels([str(num) for num in perturbation_stds])
ax1.tick_params(axis='both', which='major', labelsize=15)
ax1.set_ylim(6, 7)
plt.tight_layout()
fig1.savefig(os.path.join(fig_dir, "additional_std_optimal_cost_600dpi.png"), dpi=600)
plt.show()
...
ax2.set_xticks(perturbation_stds)
ax2.set_xticklabels([str(num) for num in perturbation_stds])
ax2.tick_params(axis='both', which='major', labelsize=15)
ax2.set_ylim(10, 50)
plt.tight_layout()
fig2.savefig(os.path.join(fig_dir, "additional_std_iterations_600dpi.png"), dpi=600)
plt.show()
