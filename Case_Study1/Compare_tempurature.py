...
temperature_values = [0.1, 0.2, 0.5, 0.7, 0.9]
...
print("\n所有 temperature 对比实验完成！")
print("最终统计结果:")
print("=" * 130)
print(f"{'温度':<10} {'平均最优点':<10} {'最优点标准差':<10} {'平均成本':<10} {'成本标准差':<10} {'平均迭代':<10} {'迭代标准差':<10} {'AvgRel G1':<10} {'G2':<10} {'G3':<10}")
print("-" * 130)

for temp in temperature_values:
    res = results[str(temp)]
    ...
    print(f"{temp:<12} {avg_point_str:<10} {std_point_str:<10} {res['avg_best_cost']:<10.4f} {res['std_best_cost']:<10.4f} {res['avg_iterations']:<10.2f} {res['std_iterations']:<10.2f} {res['avg_reliability_G1']:<10.4f} {res['avg_reliability_G2']:<10.4f} {res['avg_reliability_G3']:<10.4f}")

temps = temperature_values
optimal_costs = [results[str(n)]["avg_best_cost"] for n in temps]
optimal_costs_std = [results[str(n)]["std_best_cost"] for n in temps]
iteration_counts = [results[str(n)]["avg_iterations"] for n in temps]
iteration_counts_std = [results[str(n)]["std_iterations"] for n in temps]
...
ax1.set_xticklabels([str(num) for num in temps])
ax1.tick_params(axis='both', which='major', labelsize=15)
ax1.set_ylim(6, 7)
plt.tight_layout()
fig1.savefig(os.path.join(fig_dir, "temperature_optimal_cost_600dpi.png"), dpi=600)
plt.show()
...
ax2.set_xticklabels([str(num) for num in temps])
ax2.tick_params(axis='both', which='major', labelsize=15)
ax2.set_ylim(10, 50)
plt.tight_layout()
fig2.savefig(os.path.join(fig_dir, "temperature_iterations_600dpi.png"), dpi=600)
plt.show()
