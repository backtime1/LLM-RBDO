...
print("最终统计结果:")
print("=" * 130)
print(f"{'核采样阈值':<10} {'平均最优点':<10} {'最优点标准差':<10} {'平均成本':<10} {'成本标准差':<10} {'平均迭代':<10} {'迭代标准差':<10} {'AvgRel G1':<10} {'G2':<10} {'G3':<10}")
print("-" * 130)
...
for tp in top_p_values:
    ...
    print(f"{tp:<12} {avg_point_str:<10} {std_point_str:<10} {res['avg_best_cost']:<10.4f} {res['std_best_cost']:<10.4f} {res['avg_iterations']:<10.2f} {res['std_iterations']:<10.2f} {res['avg_reliability_G1']:<10.4f} {res['avg_reliability_G2']:<10.4f} {res['avg_reliability_G3']:<10.4f}")

tp_list = top_p_values
optimal_costs = [results[str(n)]["avg_best_cost"] for n in tp_list]
optimal_costs_std = [results[str(n)]["std_best_cost"] for n in tp_list]
iteration_counts = [results[str(n)]["avg_iterations"] for n in tp_list]
iteration_counts_std = [results[str(n)]["std_iterations"] for n in tp_list]
...
ax1.set_xlabel('Top-P', fontsize=18)
ax1.set_ylabel('Optimal Cost', fontsize=18)
ax1.legend(fontsize=14, loc='upper right')
ax1.set_xticks(tp_list)
ax1.set_xticklabels([str(num) for num in tp_list])
ax1.tick_params(axis='both', which='major', labelsize=15)
ax1.set_ylim(6, 7)
plt.tight_layout()
fig1.savefig(os.path.join(fig_dir, "top_p_optimal_cost_600dpi.png"), dpi=600)
plt.show()
...
ax2.set_xlabel('Top-P', fontsize=18)
ax2.set_ylabel('Iteration Count', fontsize=18)
ax2.legend(fontsize=14, loc='upper right')
ax2.set_xticks(tp_list)
ax2.set_xticklabels([str(num) for num in tp_list])
ax2.tick_params(axis='both', which='major', labelsize=15)
ax2.set_ylim(10, 50)
plt.tight_layout()
fig2.savefig(os.path.join(fig_dir, "top_p_iterations_600dpi.png"), dpi=600)
plt.show()
