...
adition_point_numbers = [5, 10, 20, 30]

ranges = [x1_range, x2_range]
initial_point = llm_generate_initial_points(ranges, num_points=20)
results = {}

for apn in adition_point_numbers:
    print(f"\n--- 正在测试 adition_point_number: {apn} ---")
    best_points = []
    best_costs = []
    best_rels_list = []
    iterations_list = []
    ...
    for i in range(num_runs_per_setting):
        print(f"\n--- 正在运行第 {i+1} 次 --- (adition_point_number: {apn})")
    ...
    print(f"\n--- adition_point_number {apn} 的统计结果 ---")
...
print("\n所有 adition_point_number 对比实验完成！")
print("最终统计结果:")
print("=" * 130)
print(f"{'扰动点数量':<10} {'平均最优点':<10} {'最优点标准差':<10} {'平均成本':<10} {'成本标准差':<10} {'平均迭代':<10} {'迭代标准差':<10} {'AvgRel G1':<10} {'G2':<10} {'G3':<10}")
print("-" * 130)
...
for apn in adition_point_numbers:
    ...
    print(f"{apn:<12} {avg_point_str:<10} {std_point_str:<10} {res['avg_best_cost']:<10.4f} {res['std_best_cost']:<10.4f} {res['avg_iterations']:<10.2f} {res['std_iterations']:<10.2f} {res['avg_reliability_G1']:<10.4f} {res['avg_reliability_G2']:<10.4f} {res['avg_reliability_G3']:<10.4f}")
          
perturbation_numbers = adition_point_numbers
optimal_costs = [results[str(n)]["avg_best_cost"] for n in perturbation_numbers]
optimal_costs_std = [results[str(n)]["std_best_cost"] for n in perturbation_numbers]
iteration_counts = [results[str(n)]["avg_iterations"] for n in perturbation_numbers]
iteration_counts_std = [results[str(n)]["std_iterations"] for n in perturbation_numbers]
          
# 设置图形样式，使用Times New Roman字体
...
x = np.array(perturbation_numbers)
...
ax1.set_xlabel('Number of Additional Candidates', fontsize=18)
ax1.set_ylabel('Optimal Cost', fontsize=18)
ax1.legend(fontsize=14, loc='upper right')
ax1.set_xticks(perturbation_numbers)
ax1.set_xticklabels([str(num) for num in perturbation_numbers])
ax1.tick_params(axis='both', which='major', labelsize=15)
ax1.set_ylim(6, 7)
plt.tight_layout()
fig1.savefig(os.path.join(fig_dir, "additional_candidates_optimal_cost_600dpi.png"), dpi=600)
plt.show()
...
ax2.set_xlabel('Number of Additional Candidates', fontsize=18)
ax2.set_ylabel('Iteration Count', fontsize=18)
ax2.legend(fontsize=14, loc='upper right')
ax2.set_xticks(perturbation_numbers)
ax2.set_xticklabels([str(num) for num in perturbation_numbers])
ax2.tick_params(axis='both', which='major', labelsize=15)
ax2.set_ylim(10, 50)
plt.tight_layout()
fig2.savefig(os.path.join(fig_dir, "additional_candidates_iterations_600dpi.png"), dpi=600)
plt.show()
