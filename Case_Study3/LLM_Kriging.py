...
# 创建高斯过程回归模型并进行训练
reg = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=50)
...
def objective_function(x1, x2, x3):
    return math.pi*(0.51+x2)*math.sqrt((1.2*x1/(x1+1))**2+(x2-0.51)**2)+math.pi*(x3+x2)*math.sqrt((1.2/(x1+1))**2+(x2-x3)**2)

def constraint_source(X):
    preds = reg.predict(X)
    ceq = preds - 3
    return ceq.reshape(-1, 1)
...
N = 10000 # 生成蒙特卡洛样本的数量
threshold = 0 # 约束阈值
x1_range = [0.5, 0.7] # x1的范围
x2_range = [0.2, 0.3] # x2的范围
x3_range = [0.3, 0.4] # x3的范围
reliability_target = 0.99 # 目标可靠性
...
std_dev = np.array([0.006, 0.004, 0.004])  # 目标变量的标准差
penalty_weight = 1000 # 惩罚权重
adition_point_number=10 # 新增点的数量
adition_point_std = np.array([0.006, 0.004, 0.004]) # 新增点的标准差
...
if __name__ == "__main__":
    ...
    ranges = [x1_range, x2_range, x3_range]
    initial_point = llm_generate_initial_points(ranges, num_points=40)
